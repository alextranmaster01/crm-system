...
# =============================================================================
# --- TAB 7: PROJECT MANAGEMENT (FULL VERSION - NO SHORTCUTS - FIXED NO & DOCS) ---
# =============================================================================
with t7:
    # --- 0. KHỞI TẠO BIẾN BẢO MẬT ---
    # Đảm bảo trạng thái Admin được duy trì trong suốt phiên làm việc
    if 'is_admin' not in st.session_state:
        st.session_state.is_admin = False

    # --- 1. TẢI DỮ LIỆU TỪ CƠ SỞ DỮ LIỆU ---
    # Tải danh sách dự án, chi phí và thông tin khách hàng
    df_projects = load_data("crm_projects", order_by="created_at", ascending=False)
    df_costs_master = load_data("crm_project_costs")
    cust_db = load_data("crm_customers")

    # =============================================================================
    # === THÊM CODE NÂNG CẤP: CỘT "NEEDING DOCS" (KHÔNG XÓA BẤT KỲ DÒNG NÀO CŨ) ===
    # =============================================================================
    # Kiểm tra và đảm bảo cột 'needing_docs_folder' luôn tồn tại trong DataFrame
    if 'needing_docs_folder' not in df_projects.columns:
        df_projects['needing_docs_folder'] = ""
    # =============================================================================
    ...
    with c_tab_head2:
        # Sử dụng popover để bảo mật chức năng đăng nhập Admin
        with st.popover("🔑 PHÂN QUYỀN", use_container_width=True):
            if not st.session_state.is_admin:
                pwd_v7 = st.text_input("Mật khẩu Admin", type="password", key="pwd_tab7_vfinal_fix_v12")
                if pwd_v7 == "admin123":
                ...
                if st.button("🔴 KHÓA QUYỀN", use_container_width=True, key="lock_tab7_vfinal_fix_v12"):
                    st.session_state.is_admin = False
                    st.rerun()

    # --- 3. BỨC TRANH TOÀN CẢNH (DASHBOARD) ---
    if not df_projects.empty:
        df_dash_calc = df_projects.copy()
        
        # Tính toán chi phí thực tế từ bảng crm_project_costs
        ...
        # Điền giá trị 0 cho các dự án chưa có chi phí ghi nhận
        df_dash_calc['total_cost'] = df_dash_calc['total_cost'].fillna(0)
        
        # Tính toán Lợi nhuận và % Lợi nhuận dự kiến
        ...
        # Hiển thị các thẻ KPI (Revenue, Cost, Profit)
        m1, m2, m3 = st.columns(3)
        
        # YÊU CẦU 1: Ẩn thông tin tài chính nếu không phải là tài khoản Admin
        ...
        # --- 4. BỘ LỌC & DANH SÁCH DỰ ÁN CHI TIẾT ---
        c_left, c_right = st.columns([1, 4])
        with c_left:
            st.markdown("📂 **TÊN KHÁCH HÀNG**")
            selected_cust = st.selectbox("Lọc khách hàng:", ["TẤT CẢ"] + sorted(df_dash_calc["customer_name"].dropna().unique().tolist()), key="filter_cust_v7_vfinal_full")
            ...
            sel_prj_id = st.selectbox("Xem GANTT & Chi phí:", df_filtered["project_code"].tolist(), key="sel_active_prj_v7_vfinal_full")
        ...
        with c_right:
            ...
            with col_t2:
                # --- TẠO DỰ ÁN MỚI (GIỮ NGUYÊN CHECK TRÙNG MÃ) ---
                with st.popover("➕ TẠO DỰ ÁN MỚI", use_container_width=True):
                    p_code_n = st.text_input("Mã Dự Án (VD: HS-001)", key="n_code_v7_full")
                    p_name_n = st.text_input("Tên Dự Án", key="n_name_v7_full")
                    p_cust_n = st.selectbox("Khách Hàng", [""] + cust_db["short_name"].tolist() if not cust_db.empty else [], key="n_cust_v7_full")
                    p_bud_n = st.number_input("Ngân sách (VND)", min_value=0.0, step=1000000.0, key="n_bud_v7_full")
                    c_d1, c_d2 = st.columns(2)
                    p_start_n = c_d1.date_input("Ngày Bắt Đầu", value=datetime.now(), key="n_start_v7_full")
                    p_end_n = c_d2.date_input("Ngày Kết Thúc", value=datetime.now(), key="n_end_v7_full")
                    p_img_n = st.file_uploader("🖼️ Upload ảnh dự án", type=["png", "jpg", "jpeg"], key="n_img_v7_full")

                    # YÊU CẦU 2: Ô UPLOAD NEEDING DOCS KHI TẠO DỰ ÁN MỚI (CHẤP NHẬN ĐA ĐỊNH DẠNG)
                    ...
                    if st.button("💾 LƯU DỰ ÁN", use_container_width=True, type="primary", key="btn_save_v7_full"):
                        if p_code_n and p_name_n:
                            ...
                            if existing.data:
                                ...
                                # XỬ LÝ UPLOAD TÀI LIỆU NEEDING DOCS (YÊU CẦU 2)
                                docs_folder_init = ""
                                if p_docs_n and len(p_docs_n) > 0:
                                    try:
                                        srv = get_drive_service()
                                        if srv:
                                            # Tạo cấu trúc folder trên Drive theo mã dự án
                                            ...
                                            for uploaded_file in p_docs_n:
                                                # Sử dụng hàm structured để đảm bảo tính nhất quán
                                                upload_to_drive_structured(uploaded_file, path_list, uploaded_file.name)
                                            st.success("📂 Đã upload toàn bộ tài liệu dự án lên Google Drive!")
                                ...
                                new_rec = {
                                    ...
                                    "needing_docs_folder": docs_folder_init,
                                    "status": "In Progress"
                                }
                                ...
                                try:
                                    ...
                                    st.error(f"Lỗi khi lưu dự án: {str(e)}")
                        else:
                            st.error("Vui lòng nhập đầy đủ **Mã Dự Án** và **Tên Dự Án**!")

            # --- HIỂN THỊ DANH SÁCH DỰ ÁN TRONG DATA EDITOR ---
            df_table = df_filtered[['project_image', 'project_code', 'project_name', 'start_date', 'end_date', 'status', 'budget_vnd', 'total_cost', 'profit', 'profit_pct_raw', 'needing_docs_folder']].copy()
            
            # YÊU CẦU 1: Xử lý link Needing Docs (Để trống nếu chưa có data)
            df_table['needing_docs_folder'] = df_table['needing_docs_folder'].apply(lambda x: x if (x and str(x).strip() != "" and str(x).lower() != "none") else "")

            # YÊU CẦU 4: Đổi cột "Select" thành cột "No" (Số thứ tự) và bỏ tickbox
            df_table.insert(0, "No", range(1, len(df_table) + 1))

            # Logic Cache Busting cho hình ảnh đại diện
            ...
            df_table['project_image_display'] = df_table['project_image'].apply(make_cache_busting_url)

            # Hàm định dạng hiển thị tiền tệ hoặc phần trăm dựa trên quyền Admin
            ...
            df_table['total_cost_disp'] = df_table['total_cost'].apply(lambda x: mask_data_v7(x))
            ...
            # Xác định các cột sẽ hiển thị tùy theo quyền Admin
            cols_display = ['No', 'project_image_display', 'project_code', 'project_name', 'start_date', 'end_date', 'status', 'needing_docs_folder']
            if st.session_state.is_admin:
                cols_display += ['budget_vnd_disp', 'total_cost_disp', 'profit_disp', '% Profit']

            # Vẽ bảng dữ liệu (Data Editor)
            edited_df_p = st.data_editor(
                df_table[cols_display],
                column_config={
                    # YÊU CẦU 4: Cột No nhỏ gọn đủ chứa 2-3 chữ số
                    "No": st.column_config.NumberColumn("No", width=35, help="Số thứ tự"),
                    "project_image_display": st.column_config.ImageColumn("Hình ảnh", width="medium", help="Ảnh đại diện dự án"),
                    ...
                    # YÊU CẦU 1: Cột Link dẫn đến Drive Folder
                    "needing_docs_folder": st.column_config.LinkColumn(
                        "Needing Docs",
                        width="medium",
                        help="Nhấn để xem toàn bộ tài liệu trên Drive (Word, Excel, PDF, Video...).",
                        display_text="📂 Xem tài liệu"
                    )
                },
                use_container_width=True,
                hide_index=True,
                key="prj_editor_v7_final_no_checkbox_full"
            )

            # --- NÚT XÓA DỰ ÁN (CHỈ DÀNH CHO ADMIN) ---
            if st.session_state.is_admin:
                with st.popover("🗑️ Xóa dự án", help="Xóa dự án và các dữ liệu liên quan"):
                    st.markdown("**Chọn dự án cần xóa bỏ khỏi hệ thống**")
                    project_options = ["Chọn dự án..."] + df_filtered["project_code"].tolist()
                    selected_to_delete = st.selectbox("Danh sách dự án:", project_options, key="select_delete_prj_v7_full")
                    if selected_to_delete != "Chọn dự án...":
                        st.warning(f"Bạn đang chọn xóa dự án: **{selected_to_delete}**")
                        st.info("Hành động này sẽ xóa cả Nhiệm vụ và Chi phí liên quan!")
                        delete_pwd = st.text_input("Nhập mật khẩu xác nhận xóa:", type="password", key="pwd_delete_select_v7_full")
                        if st.button("🔥 Xác nhận xóa vĩnh viễn", type="primary", use_container_width=True):
                            if delete_pwd == "admin123":
                                with st.spinner("Đang thực hiện xóa dự án..."):
                                    try:
                                        ...
                                        st.error(f"Lỗi khi thực hiện lệnh xóa: {str(e)}")
                            else:
                                st.error("Mật khẩu Admin không chính xác!")

        # --- 5. QUẢN LÝ CHI TIẾT (GANTT, CHI PHÍ, CÀI ĐẶT) ---
        if sel_prj_id:
            ...
            # Tải dữ liệu nhiệm vụ cho GANTT
            ...
            # Khởi tạo danh sách các Tab chi tiết
            ...
            # TAB 1: BIỂU ĐỒ GANTT VÀ NHIỆM VỤ
            with tabs[0]:
                col_g1, col_g2 = st.columns([2, 3])
                
                # Tính toán tiến độ trung bình của toàn dự án
                ...
                with col_g1:
                    if not tasks_data.empty:
                        # Vẽ biểu đồ GANTT bằng thư viện Altair
                        ...
                        # Dòng Master đại diện cho tổng tiến độ dự án
                        m_row = pd.DataFrame([{
                        ...
                        chart = alt.Chart(pd.concat([m_row, df_g])).mark_bar(cornerRadius=5, height=20).encode(
                            ...
                            x2='end_date',
                            y=alt.Y('task_name', sort=None, title='Tên nhiệm vụ'),
                        ...
                        st.info("Chưa có nhiệm vụ nào được ghi nhận. Vui lòng thêm nhiệm vụ bên phải.")

                with col_g2:
                    # Bảng chỉnh sửa nhiệm vụ trực tiếp
                    ...
                    edited_tasks = st.data_editor(
                        ...
                        key=f"ed_v7_task_full_up_{sel_prj_id}"
                    )
                    
                    if st.button("💾 Cập nhật Tiến độ & Nhiệm vụ", type="primary", use_container_width=True, key=f"btn_up_tasks_{sel_prj_id}"):
                        with st.spinner("Đang đồng bộ nhiệm vụ..."):
                            try:
            ...
            # TAB 2: QUẢN LÝ CHI TIẾT CHI PHÍ (CHỈ ADMIN)
            if st.session_state.is_admin:
                with tabs[1]:
                    ...
                    edited_costs = st.data_editor(
                        ...
                        column_config={
                            ...
                            "description": st.column_config.TextColumn("Mô tả chi tiết")
                        },
                        key=f"ed_v7_cost_full_up_{sel_prj_id}"
                    )

                    if st.button("💾 Cập nhật Chi phí dự án", type="primary", use_container_width=True, key=f"btn_up_costs_{sel_prj_id}"):
                        with st.spinner("Đang lưu chi phí..."):
                            try:
                                ...
                                st.success("✅ Cập nhật chi phí thành công!")
                                time.sleep(0.8)
                                st.rerun()
                            except Exception as e:
                ...
                with tabs[2]:
                    ...
                    with st.form(key=f"edit_f_vfinal_full_up_{sel_prj_id}"):
                        ...
                        u_s = c_e1.date_input("Bắt Đầu", value=pd.to_datetime(active_prj['start_date']) if active_prj['start_date'] else datetime.now())
                        u_e = c_e2.date_input("Kết Thúc", value=pd.to_datetime(active_prj['end_date']) if active_prj['end_date'] else datetime.now())
                        ...
                        if st.form_submit_button("💾 XÁC NHẬN CẬP NHẬT TOÀN BỘ", use_container_width=True, type="primary"):
                            ...
                            # Xử lý thay thế ảnh cũ nếu có ảnh mới
                            if u_img:
                                with st.spinner("Đang cập nhật ảnh đại diện mới..."):
                                    timestamp = int(time.time())
                                    ...
                                    up_payload["project_image"] = new_img_url

                            # YÊU CẦU 3: Xử lý cập nhật tài liệu lên Drive (Ghi đè nếu trùng)
                            if u_docs and len(u_docs) > 0:
                                with st.spinner("Đang đồng bộ tài liệu lên Drive (xử lý ghi đè)..."):
                                    try:
                                        srv = get_drive_service()
                                        if srv:
                                            path_list = ["CRM_PROJECT_DOCS", active_prj['project_code'].strip().upper()]
                                            folder_id = get_or_create_folder_hierarchy(srv, path_list, ROOT_FOLDER_ID)
                                            # Lưu lại link folder vào database
                                            new_docs_folder = f"https://drive.google.com/drive/folders/{folder_id}"
                                            for uploaded_file in u_docs:
                                                # Hàm upload_to_drive_structured mặc định sẽ cập nhật nội dung nếu file tồn tại
                                                upload_to_drive_structured(uploaded_file, path_list, uploaded_file.name)
                                            up_payload["needing_docs_folder"] = new_docs_folder
                                            st.success("📂 Tài liệu đã được cập nhật và ghi đè thành công trên Drive!")
                                    except Exception as e_docs:
                                        st.error(f"Lỗi khi upload tài liệu: {e_docs}")

                            # Thực hiện cập nhật vào Database Supabase
                            try:
                                supabase.table("crm_projects").update(up_payload).eq("project_code", active_prj['project_code']).execute()
                                st.cache_data.clear()
                                st.success("✅ Dự án đã được cập nhật thành công!")
                                time.sleep(1.5)
                                st.rerun()
                            except Exception as e:
        ...
        st.info("Hệ thống chưa có dự án nào. Hãy nhấn 'TẠO DỰ ÁN MỚI' để bắt đầu.")
# --- KẾT THÚC TAB 7 ---
# =============================================================================
