    ...
    with ti:
        ...
        if up_i and st.button("🚀 CẬP NHẬT IMPORT DATA"):
            try:
                ...
                st.error(f"Lỗi Import: {e}")
# =============================================================================
# --- TAB 7: PROJECT MANAGEMENT (PHIÊN BẢN CÓ NÚT PHÂN QUYỀN GÓC PHẢI) ---
# =============================================================================
with t7:
    # --- 0. KHỞI TẠO & NÚT PHÂN QUYỀN (VỊ TRÍ TRÊN CÙNG BÊN PHẢI TAB) ---
    if 'is_admin' not in st.session_state:
        st.session_state.is_admin = False

    # Tạo hàng tiêu đề Tab và Nút Phân quyền
    c_tab_head1, c_tab_head2 = st.columns([8, 2])
    with c_tab_head1:
        st.markdown("### 🚀 TRUNG TÂM QUẢN LÝ DỰ ÁN (PROJECT COMMAND CENTER)")
    with c_tab_head2:
        # Nút Phân quyền nằm ở đây để Alex dễ tìm
        with st.popover("🔑 PHÂN QUYỀN", use_container_width=True):
            if not st.session_state.is_admin:
                pwd_v7 = st.text_input("Mật khẩu Admin", type="password", key="pwd_v7_final")
                if pwd_v7 == "admin123":
                    st.session_state.is_admin = True
                    st.success("Đã mở quyền!")
                    time.sleep(0.5); st.rerun()
            else:
                st.info("🔓 Đang có quyền Admin")
                if st.button("🔴 KHÓA QUYỀN", use_container_width=True, key="lock_v7_final"):
                    st.session_state.is_admin = False
                    st.rerun()
    ...
    if not df_projects.empty:
        ...
        with c_left:
            ...
            selected_cust = st.selectbox("Lọc khách hàng:", ["TẤT CẢ"] + sorted(df_dash_calc["customer_name"].unique().tolist()), key="filter_v7_p")
            ...
            sel_prj_id = st.selectbox("Xem GANTT & Chi phí:", df_filtered["project_code"].tolist(), key="sel_prj_v7_p")
        ...
        with c_right:
            ...
            with col_t2:
                with st.popover("➕ TẠO DỰ ÁN MỚI", use_container_width=True):
                    p_code_new = st.text_input("Mã Dự Án", key="p_code_new")
                    p_name_new = st.text_input("Tên Dự Án", key="p_name_new")
                    p_cust_new = st.selectbox("Khách Hàng", [""] + cust_db["short_name"].tolist() if not cust_db.empty else [], key="p_cust_new")
                    p_bud_new = st.number_input("Ngân sách (VND)", min_value=0.0, key="p_bud_new")
                    if st.button("💾 LƯU", use_container_width=True, type="primary"):
                        supabase.table("crm_projects").insert([{"project_code": p_code_new.upper(), "project_name": p_name_new, "customer_name": p_cust_new, "budget_vnd": p_bud_new, "status": "In Progress"}]).execute()
                        st.rerun()
            ...
            df_table['total_cost'] = df_table['total_cost'].apply(lambda x: mask_func(x))
            df_table['profit'] = df_table['profit'].apply(lambda x: mask_func(x))
            df_table['% Profit'] = df_table['profit_pct_raw'].apply(lambda x: mask_func(x, False))

            st.dataframe(df_table.drop(columns=['profit_pct_raw']), column_config={"project_image": st.column_config.ImageColumn("Hình ảnh")}, use_container_width=True, hide_index=True)
        ...
        if sel_prj_id:
            ...
            tasks_data = tasks_data[tasks_data["project_code"] == sel_prj_id] if not tasks_data.empty else pd.DataFrame()
