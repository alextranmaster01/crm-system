    ...
    with ti:
        ...
        if up_i and st.button("🚀 CẬP NHẬT IMPORT DATA"):
            try:
                ...
                st.error(f"Lỗi Import: {e}")
# =============================================================================
# --- TAB 7: PROJECT MANAGEMENT (FULL VERSION - FIX XÓA TICKBOX & ẢNH DUY NHẤT) ---
# =============================================================================
with t7:
    # --- 1. TẢI DỮ LIỆU ---
    ...
    if not df_projects.empty:
        ...
        with c_right:
                        ...
                        if p_code and p_name:
                            ...
                            if p_img_f:
                                with st.spinner("Đang tải ảnh lên Drive..."):
                                    # Thuật toán tên file cố định theo mã dự án để ghi đè vị trí cũ
            ...
            df_table.insert(0, "Select", False) # Thêm cột chọn dự án
            ...
            edited_df_prj = st.data_editor(
                ...
                use_container_width=True, hide_index=True, key="prj_editor_v7_final_fix_sync_v2"
            )

            # --- CHỨC NĂNG XÓA DÀNH CHO ADMIN (HIỆN KHI CÓ TICK BOX) ---
            ...
            if not selected_rows.empty and st.session_state.get('is_admin', False):
                ...
                with col_del1:
                    pass_conf_admin = st.text_input("XÁC NHẬN MẬT KHẨU ADMIN ĐỂ XÓA:", type="password", key="pass_confirm_del_v7_final")
                with col_del2:
                    if st.button("🔥 XÓA DỰ ÁN ĐÃ CHỌN", type="primary", use_container_width=True):
                        if pass_conf_admin == "admin123":
        ...
        if sel_prj_id:
            ...
            with tabs[0]: # TAB TIẾN ĐỘ
                ...
                with col_g2:
                    df_ed_v2 = tasks_data[["task_name", "assignee", "start_date", "end_date", "progress_pct", "status"]].copy() if not tasks_data.empty else pd.DataFrame(columns=["task_name", "assignee", "start_date", "end_date", "progress_pct", "status"])
                    df_ed_v2['start_date'] = pd.to_datetime(df_ed_v2['start_date'], errors='coerce').dt.date
                    df_ed_v2['end_date'] = pd.to_datetime(df_ed_v2['end_date'], errors='coerce').dt.date
                    ed_v7_v2 = st.data_editor(df_ed_v2, num_rows="dynamic", use_container_width=True, hide_index=True, 
                        column_config={
                        ...
                        }, key=f"ed_v7_final_v2_{sel_prj_id}")
                    if not df_ed_v2.equals(ed_v7_v2):
                        supabase.table("crm_project_tasks").delete().eq("project_code", sel_prj_id).execute()
                        save_l = [{"project_code": sel_prj_id, "task_name": r['task_name'], "assignee": r['assignee'], "start_date": str(r['start_date']), "end_date": str(r['end_date']), "progress_pct": r['progress_pct'], "status": r['status']} for r in ed_v7_v2.to_dict('records') if r['task_name']]
            ...
            if st.session_state.get('is_admin', False) and len(tabs) > 1:
                with tabs[1]: # TAB CHI PHÍ
                    ...
                    ed_c = st.data_editor(df_c_disp, num_rows="dynamic", use_container_width=True, hide_index=True, key=f"cs_ed_tab7_v2_{sel_prj_id}")
                ...
                with tabs[2]: # TAB CÀI ĐẶT (THAY THẾ ẢNH DUY NHẤT)
                    ...
                    with st.form(key=f"edit_prj_v2_{sel_prj_id}"):
                        ...
                        u_img = st.file_uploader("Thay đổi ảnh dự án (Sẽ đẩy ảnh cũ ra - Thay bằng ảnh mới)", type=["png", "jpg", "jpeg"])
                        ...
                        if st.form_submit_button("💾 CẬP NHẬT & THAY THẾ ẢNH", use_container_width=True):
                            ...
                            if u_img:
                                with st.spinner("Đang đẩy ảnh cũ ra và thay bằng ảnh mới..."):
                                    # Gọi hàm upload để ghi đè vào vị trí file cũ (Sử dụng project_code của dự án hiện tại)
                            ...
                            try:
                                ...
                                st.cache_data.clear() # Làm mới bộ nhớ đệm để ảnh hiện lên ngay
