    ...
    with ti:
        ...
        if up_i and st.button("🚀 CẬP NHẬT IMPORT DATA"):
            try:
                ...
                st.error(f"Lỗi Import: {e}")
# =============================================================================
# --- TAB 7: PROJECT MANAGEMENT (PHIÊN BẢN FIX XÓA TICKBOX & THAY THẾ ẢNH) ---
# =============================================================================
with t7:
    # --- 1. TẢI DỮ LIỆU ---
    ...
    with c_tab_head2:
        with st.popover("🔑 PHÂN QUYỀN", use_container_width=True):
            if not st.session_state.get('is_admin', False):
                pwd_input = st.text_input("Mật khẩu Admin", type="password", key="pwd_tab7_v26_final_sync")
    ...
    if not df_projects.empty:
        ...
        with c_right:
                        ...
                        if p_code and p_name:
                            ...
                            if p_img_f:
                                with st.spinner("Đang lưu ảnh..."):
                                    # LOGIC THAY THẾ: Luôn đặt tên theo mã dự án
            ...
            # --- PHẦN 2: HIỂN THỊ DANH SÁCH & LOGIC XÓA (FIX 100%) ---
            ...
            # Thuật toán thay thế ảnh hiển thị ngay tức thì
            ...
            # --- SỬA LỖI NHẤP NHÁY: Tách data_editor để xử lý logic xóa ---
            edited_df_prj = st.data_editor(
                ...
                use_container_width=True, hide_index=True, key="prj_editor_v7_final_sync_100"
            )

            # Lấy danh sách dòng được tick
            ...
            if not selected_to_del.empty and st.session_state.get('is_admin', False):
                st.warning(f"🛑 Đang chọn xóa {len(selected_to_del)} dự án. Thao tác không thể khôi phục!")
                col_p_del, col_b_del = st.columns([3, 1])
                with col_p_del:
                    pass_admin = st.text_input("XÁC NHẬN MẬT KHẨU ADMIN ĐỂ XÓA:", type="password", key="pwd_confirm_del_v100")
                with col_b_del:
                    if st.button("🔥 XÓA DỰ ÁN ĐÃ CHỌN", type="primary", use_container_width=True):
                        if pass_admin == "admin123":
                            ids = selected_to_del["project_code"].tolist()
                            # Xóa sạch dữ liệu liên quan ở 3 bảng
                            supabase.table("crm_projects").delete().in_("project_code", ids).execute()
                            supabase.table("crm_project_tasks").delete().in_("project_code", ids).execute()
                            supabase.table("crm_project_costs").delete().in_("project_code", ids).execute()
                            st.cache_data.clear()
        ...
        if sel_prj_id:
            ...
            with tabs[0]: # TAB TIẾN ĐỘ
                ...
                with col_g2:
                    ...
                    if not df_ed_t.equals(ed_v7):
                        ...
                        st.toast("🚀 Đã cập nhật tiến độ!", icon="✅"); time.sleep(0.3); st.rerun()
            ...
            if st.session_state.get('is_admin', False) and len(tabs) > 1:
                with tabs[1]: # TAB CHI PHÍ
                    ...
                    ed_c = st.data_editor(df_c_disp, num_rows="dynamic", use_container_width=True, hide_index=True, key=f"cs_ed_tab7_v100_{sel_prj_id}")
                ...
                with tabs[2]: # TAB CÀI ĐẶT (THAY THẾ ẢNH 100%)
                    st.markdown("### ⚙️ CÀI ĐẶT THÔNG TIN DỰ ÁN")
                    with st.form(key=f"edit_f_v100_{sel_prj_id}"):
                        ...
                        u_img = st.file_uploader("Thay đổi ảnh dự án (Ghi đè bản cũ)", type=["png", "jpg", "jpeg"])
                        
                        if st.form_submit_button("💾 CẬP NHẬT & THAY THẾ ẢNH", use_container_width=True):
                            # LOGIC DUY NHẤT: Ghi đè file cũ trên Drive
                            ...
                            if u_img:
                                with st.spinner("Đang đẩy ảnh cũ ra và thay bằng ảnh mới..."):
                                    img_url_up, _ = upload_to_drive_simple(u_img, "CRM_PROJECT_IMAGES", f"PRJ_NAME_{active_prj['project_code'].strip().upper()}.png")
                            ...
                            try:
                                ...
                                st.cache_data.clear() # Ép trình duyệt load lại ảnh mới
